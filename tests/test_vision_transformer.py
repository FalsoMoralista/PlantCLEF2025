...
ViT = VisionTransformer(img_size=[2048,1024], pretrained_patch_embedder=model,patch_size=patch_size, embed_dim=768, depth=8)
ViT.to(device)
...
for im_id, (preprocessed_images, _, name) in enumerate(test_dataloader):
    ...
    y = prototypes # torch.randn(batch_size, 768).to(device)
