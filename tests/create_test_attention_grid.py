...
from src.helper import (
    visualize_global_attention_on_image,
...
cid_to_spid = load_class_mapping(
    "/home/rtcalumby/adam/luciano/PlantCLEF2025/PlantCLEF2025/pretrained_models/class_mapping.txt"
)
spid_to_sp = load_species_mapping(
    "/home/rtcalumby/adam/luciano/PlantCLEF2025/PlantCLEF2025/pretrained_models/species_id_to_name.txt"
)
...
model.head = (
    torch.nn.Identity()
)  # Replace classification head by identity layer for feature extraction
print("Cls Head:", cls_head)

dino_cls = timm.create_model(
    "vit_base_patch14_reg4_dinov2.lvd142m",
    pretrained=False,
    num_classes=len(cid_to_spid),
    checkpoint_path=pretrained_path,
...
test_dataset, test_dataloader, dist_sampler = build_test_dataset(
...
for epoch_no in epochs:
    checkpoint = torch.load(
        r_path + f"experiment_0-ep{epoch_no}.pth.tar", map_location=torch.device("cpu")
    )
    ViT = PilotVisionTransformer(
        img_size=[2048, 2048],
        pretrained_patch_embedder=model,
    ...
    for itr, (img_tensor, _, image_name) in enumerate(test_dataloader):
        ...
        original_image = Image.open(test_dir + "test/" + image_name)
        original_image.show()
        ...
        n_kmeans = [
            KMeansModule(K=k, dimensionality=768, config=config, resources=resources)
            for k in k_range
        ]
        ...
        best_K, cluster_assignments = cosine_cluster_index(
            n_kmeans=n_kmeans,
            k_range=k_range,
        ...
        visualize_clustered_crops(
            selected_crops=selected_crops.cpu(),
            cluster_assignments=cluster_assignments.cpu(),
            image_name=image_name,
            save_path="attention/pilot",
        )
        exit(0)
    print("predicted items length:", len(logit_list))
    # torch.save(
    #     logit_list, f"logits/clustered_preds_run_0_ep15_threshold={threshold}.pt"
    # )
